...
four-stage profit-driven feature selector that reduces 500 input variables to six, complemented
by calibrated gradient-boosted tree ensembles and a threshold optimised directly against the
...
achieves a cross-validated expected score of \textbf{6\,070 EUR} and targets all 1\,000
available campaign slots.
\end{abstract}

\section{Problem Formulation}

A bank wants to contact up to $N_{\max} = 1\,000$ customers from a pool of 5\,000 with a
...
\textbf{Variable cost implications.}
Each additional feature must increase the expected customer profit by at least 200 EUR to be
worth including. For a model yielding 78\% precision on 1\,000 targets, the variable budget
is roughly $\lfloor (780\cdot10 - 220\cdot5)/200 \rfloor = 27$ features at most before the
data cost exceeds the revenue. In practice, weaker variables should be excluded far earlier,
since the precision estimate itself degrades as irrelevant features are added.
...
\draw[arr] (s2) -- (s3);
\draw[arr] (s3) -- (out);
\node[below=0.12cm of s0, font=\scriptsize, color=gray] {500 $\to$ $\sim$400};
...
by mean decrease in impurity, reducing dimensionality from ${\sim}400$ to~50.
...
The HGB model on Boruta features achieves 68.0\% validation accuracy, far above the 53.8\%
of the single-feature baseline, yet yields a lower financial score by nearly 5\,000 EUR.
...
\midrule
\textbf{Proposed pipeline} & \textbf{6} & \textbf{1\,200} & \textbf{0.744} & \textbf{1\,745} \\
\bottomrule
\end{tabular}
\end{table}

\noindent The Boruta results are particularly instructive: even a state-of-the-art feature
selector designed for Random Forests produces a negative profit because the 5\,200 EUR data
cost eclipses any benefit from selecting 26 genuinely predictive variables. This confirms that
...
Having paid for $K = 6$ original variables, we can generate derived features at no additional
cost. We constructed: $\binom{6}{2} = 15$ pairwise products, $6 \times 5 = 30$ directed
ratios, 6 signed-log transforms $\mathrm{sign}(x)\log(1+|x|)$, 5 row statistics (min, max,
mean, std, range), 2 PCA components, and 5 K-means features (distances to 4 centroids and
cluster label) --- 63 derived features in total. Models trained on the full 69-dimensional
space were compared against the baseline 6-feature model via CV profit. No architecture
(cautious HGB, expressive HGB, RF, LR) improved over the 6-feature baseline, and the same
six original variables were re-selected in follow-up runs. This indicates that the original
...
\midrule
Standard   & 50  & 15 & 0.85 & 6 & \textbf{6\,070} \\
Wide net   & 100 & 30 & 0.90 & 3 &           5\,440 \\
Strict net & 30  & 10 & 0.80 & 6 & \textbf{6\,070} \\
\bottomrule
\end{tabular}
\end{table}

The Standard and Strict configurations converge to the \emph{identical} set of six features
(V11, V160, V191, V215, V342, V380) despite different Stage~1--2 widths and correlation
thresholds. The Wide-net run, which relaxes the correlation filter, selects only three
variables; V224 and V390 overlap substantially in information content with members of the
six-feature set, so Stage~3 stops earlier. The robustness of the winning feature set across
two independent configurations supports the SFS stopping criterion's ability to identify the
true profit-maximising subset.
...
\begin{table}[H]
\centering
\caption{Final model performance. Classification metrics are reported on the 1\,000-sample
holdout set; CV profit is the 5-fold OOF estimate on all 5\,000 training samples.}
\label{tab:final}
\begin{tabular}{@{}lr@{}}
\toprule
Metric & Value \\
...
ROC-AUC   & 0.817 \\
\midrule
Profit (holdout) & 1\,745 EUR \\
\textbf{Profit (5-fold CV, 5\,000 samples)} & \textbf{6\,070 EUR} \\
\bottomrule
\end{tabular}
\end{table}

The selected variables V11, V160, V191, V215, V342, V380 represent less than 1.2\% of the
original feature space; their total acquisition cost is 1\,200 EUR. The gap between holdout
profit (1\,745 EUR) and CV profit (6\,070 EUR) reflects the training set size available in
each evaluation: the holdout model uses 4\,000 samples, whereas each CV fold uses 4\,000
training and 1\,000 validation samples, making the two protocols directly comparable in
scale. The final submitted model is trained on all 5\,000 observations. At 78\% precision
with 1\,000 targeted customers, the expected test score is approximately:
\[
    780 \times 10 - 220 \times 5 - 6 \times 200 = 7{,}800 - 1{,}100 - 1{,}200 = \mathbf{5{,}500~\text{EUR}}.
\]
...
criterion yields a competitive result with only six variables.
Second, the stability of the selected feature set across independent configurations
(Standard and Strict) and across five alternative pipeline variants provides strong evidence
that the solution is not an artefact of a particular search trajectory.
Third, free feature engineering on the six purchased variables did not improve profit,
