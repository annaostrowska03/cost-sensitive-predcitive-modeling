...
business objective. Experiments across baseline methods, free feature engineering, and alternative classifier
architectures show that embedding the profit criterion directly into the selection process
is the key design choice. The resulting pipeline
...
The 200 EUR penalty per variable has a large impact on model complexity. For a new feature to break even, it must independently yield at least 20 extra true positives (20 TP $\times$ 10 EUR = 200 EUR), which is a high bar given the noise level in this dataset.

In our experiments, adding variables beyond four consistently reduced net profit. This points to a very compact feature set as the optimal strategy.
...
Individual Pearson correlations with the label are uniformly weak: the strongest single
...
\node[below=0.12cm of s1, font=\scriptsize, color=gray] {$\to$ 55};
...
The decision threshold $\tau$ is optimised by grid search over
$\mathcal{T} = \{0.15,\, 0.15 + \Delta\tau,\, \ldots,\, 0.50\}$ with 36 equally spaced points
directly on the out-of-fold predictions:
\begin{equation}
...
Because the threshold is calibrated directly on the raw OOF probability scores, the final model
is intentionally left uncalibrated so that the score distribution at inference time matches the
one seen during threshold selection.
...
\begin{itemize}
    \item \emph{HGB-only:} best-tuned HGB model, applied directly without probability calibration
          so that threshold selection and final scoring use the same score distribution.
    \item \emph{Profit-proportional soft-voting ensemble:} HGB, Random Forest, and
          $\ell_2$-regularised Logistic Regression, each weighted proportionally to its
          standalone OOF CV profit (yielding approximately $w_\text{HGB}{=}0.55$,
          $w_\text{RF}{=}0.30$, $w_\text{LR}{=}0.15$ in our experiments).
\end{itemize}
The strategy with higher OOF CV profit is retained.
...
LR --- top-5 correlated features  & 5  & 1\,000 & 0.557 & 1\,490 \\
...
5\,000 observations.  The OOF CV estimate implies approximately 73\% precision on
1\,000 targeted customers (731 TP, 269 FP), consistent with the reported profit:
\[
    731 \times 10 - 269 \times 5 - 4 \times 200 = 7{,}310 - 1{,}345 - 800 = \mathbf{5{,}165~\text{EUR}}.
\]
