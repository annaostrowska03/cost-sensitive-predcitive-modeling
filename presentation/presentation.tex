...
\midrule
\textbf{Proposed pipeline}      & \textbf{6} & \textbf{1\,200 EUR} & \textbf{0.744} & \textbf{+1\,745 EUR} \\
\bottomrule
\end{tabular}
\end{center}
...
\node[below=0.10cm of s3, font=\tiny, color=mid] {6};
\end{tikzpicture}
\end{center}
...
\midrule
Standard   & 50  & 15 & 0.85 & 6 & \textbf{6\,070 EUR} \\
Wide net   & 100 & 30 & 0.90 & 3 & 5\,440 EUR \\
Strict net & 30  & 10 & 0.80 & 6 & \textbf{6\,070 EUR} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.4em}
Standard and Strict converge to the same six features:
\begin{center}
\textbf{V11 \quad V160 \quad V191 \quad V215 \quad V342 \quad V380}
\end{center}
...
RF-only (tuned)               & 4  & $<$6\,070 EUR (CV) & \\
Calibrated HGB + thr.\ search & 4  & $<$6\,070 EUR (CV) & \\
Free feature engineering      & 6  & $\approx$6\,070 EUR (CV) & no improvement \\
\textbf{Main pipeline}        & \textbf{6} & \textbf{6\,070 EUR (CV)} & \textbf{winner} \\
\bottomrule
\end{tabular}
\end{center}
...
\slabel{Selected feature set ($K=6$)}
\begin{center}
\textbf{V11 \; V160 \; V191 \; V215 \; V342 \; V380}
\end{center}
\begin{itemize}\itemsep2pt
  \item $<$1.2\% of the original 500 features
  \item Data cost: $6\times200 = 1\,200$ EUR
  \item 1\,000 customers targeted (full budget)
  \item Decision threshold: $\tau^* = 0.333$
\end{itemize}

\column{0.48\textwidth}

\slabel{Metrics (1\,000-sample holdout)}
\footnotesize
\begin{tabular}{@{}lr@{}}
\toprule
Metric & Value \\
...
ROC-AUC   & 0.817 \\
\midrule
Holdout profit           & 1\,745 EUR \\
\textbf{CV profit (5-fold)} & \textbf{6\,070 EUR} \\
\bottomrule
\end{tabular}

\vspace{0.3em}
{\tiny\itshape Holdout trains on 4\,000 samples; final model uses all 5\,000.}
...
\[
  780{\times}10 - 220{\times}5 - 6{\times}200 = \mathbf{5{,}500~\text{EUR}}
\]
...
\textbf{Proposed} & \textbf{$\approx$+5\,500 EUR} \\
\bottomrule
\end{tabular}
...
\begin{enumerate}\itemsep3pt\small
  ...
  \item \textbf{Stable solution.} Two independent configs + 5 alternative pipelines all converge to the same 6 features.
  \item \textbf{Free features add nothing.} 63 derived features gave no CV gain.
  \item \textbf{More vars $\neq$ better.} 56-var L1 scores $-$8\,750 EUR; 6 vars score $+$5\,500 EUR.
\end{enumerate}
